...
function merge_sort(left, mid, right)
{
  ...
  for (var i = left; i <= right; i++) {
    ...
    if (l > mid) {
      logBox.updateLogBox(sortingArray.arrayBarSizes[l], sortingArray.arrayBarSizes[m], "Comparing", c_delay+=delay);
      Arr[k++] = sortingArray.arrayBarSizes[m++];
      sortingArray.updateDivElement(m-1, "red", c_delay+=delay);
    }
    else if (m > right) {
      logBox.updateLogBox(sortingArray.arrayBarSizes[m], sortingArray.arrayBarSizes[right], "Comparing", c_delay+=delay);
